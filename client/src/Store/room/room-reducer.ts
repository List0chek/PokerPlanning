import { ICard, IDiscussion, IRoom, IVote } from '../types';
...
const initState: IRoom = {
  ...
  discussions: [
    {
      id: '456',
      roomId: '8f37fada-1bcd-4029-bd70-2f51d7b6182d',
      topic: 'topicName1',
      ...
      votes: [
        {
          ...
          roomId: '8f37fada-1bcd-4029-bd70-2f51d7b6182d',
          discussionId: '476f9f57-dd4e-41e0-81ff-5dbe876400ac',
          user: {
            id: 'd92aa5b4-290e-495a-909d-a26ea4ca068a',
            name: 'userName 3',
            token: 'tKUq2Q4pWkmQnaJupMoGig==',
...
export function roomReducer(state: IRoom = initState, action: IVoteAction): IRoom {
  switch (action.type) {
    case ActionType.VOTE: {
      const voteId = Math.round(Math.random() * (100 - 1) + 1);
      const newVote: IVote = {
        id: voteId.toString(),
      ...
      return {
        ...
        discussions: state.discussions.map((discussion) => {
          if (discussion.id === action.discussionId) {
            if (!discussion.votes.find((item) => item.user === action.user)) {
              return {
                ...discussion,
